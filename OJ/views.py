...
def dashboardPage(request):
    ...
    hard_ques_count = len(Problem.objects.filter(difficulty="Hard"))

    user = request.user
    easy_solve_count = user.easy_solve_count
    medium_solve_count = user.medium_solve_count
    hard_solve_count = user.hard_solve_count
...
# Shows problem description of left side and has a text editor on right side with code submit buttton.
@login_required(login_url='login')
...
def verdictPage(request, problem_id):
    if request.method == 'POST':
        ...
        if language == "C++":
            ...
            compile = "python3"
            clean = f"{filename}.py"
            docker_img = "python:3.9.18-slim-bullseye"
        ...
        if cmp.returncode != 0:
            ...
            try:
                if language == "Python3":
                    res = subprocess.run( ["docker", "exec", "-i", cont_name, "python",file], cwd="/", input=testcase.input.encode(), capture_output=True, timeout=problem.time_limit)
                else:
                    res = subprocess.run( ["docker", "exec", "-i", cont_name, exe], cwd="/", input=testcase.input.encode(), capture_output=True, timeout=problem.time_limit)
